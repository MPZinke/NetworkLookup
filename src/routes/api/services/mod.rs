...
{
	let query_response: Result<Vec<Service>, ResponseError> = get_services(pool.as_ref()).await;
...
pub async fn id(path: web::Path<i32>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<Service, ResponseError> = get_service_by_id(pool.as_ref(), id).await;
	return query_to_response(query_response);
}
